...
import { registerCorrespondenceInHost } from "../src/application/modules/intake/registerCorrespondence.js";
...
test("registerCorrespondenceInHost persists outgoing correspondence", async () => {
    ...
    const result = await registerCorrespondenceInHost(adapter, receptionist, {
...
test("registerCorrespondenceInHost rejects department outside selected branch policy", async () => {
    const adapter = createInMemoryHostAdapter();
    const receptionist = getReceptionist();
    await assert.rejects(() => registerCorrespondenceInHost(adapter, receptionist, {
        subject: "Invalid branch mapping",
        branchId: "b-002",
        departmentId: "d-001",
        direction: "INCOMING"
    }, "BANK"), /not allowed for this branch/);
});
